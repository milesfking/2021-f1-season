...
This task was similar to task 2, but this time looking at constructors instead of the drivers. Since each constructor had two drivers, the main challenge for task 3 was finding the total points for the constructors for each individual race. To do this I grouped the dataframe from task 1 by both `constructor` and `race_name` before mutating a new `constructor_points` variable. This variable is just the sum of the points category for each constructor by race. Since there are two drivers per constructor, I used `distinct()` to ensure that there is only one entry for each constructor for each race. Our dataframe is now 220 rows, which makes sense (as 10 constructors times 22 races is 220 entries in total).
...
To make the line charts more visually appealing, I used a custom theme found online and made slight modifications (changing font sizes, adjusting plot margins, etc.) to give myself more control over the final product. Similar to what we discussed in class, I also removed the legend and opted to use labels closer to the lines to reduce the need to look back and forth between the legend and the visualizations.

Again, my visualization is primarily focused on the race for first place in both the driver and constructor standings. As such, I increased the visual presence of the top 2 competitors (by making their lines colored, increasing line size, and adding labels) while minimizing the influence of lower-ranked drivers and constructors (reducing line size, making the lines gray, and including no labels). I also named the titles strategically to let viewers know exactly what the visualizations are trying to convey. While all of the relevant numeric data from the f1 object is present on the graph, I only highlighted things that would further my narrative that this years' F1 championship was a two-way race (in both driver and constructor).
